...
class ImagePreprocessing:
    def __init__(self, directory_path, WORKING_DIR, pat_directory, JHU_ATLAS_TEMPLATE_NII_PATH, EXCEL_INFO):
    ...
    def create_patients_mni_folders(self):
        for idx in range(len(self.patients_list)):
            path = os.path.join(self.WORKING_DIR, 'pat_directory', self.patients_list[idx], 'mnispace')
    ...
    def create_patients_work_folders(self):
        for idx in range(len(self.patients_list)):
            path_workspace = os.path.join(self.WORKING_DIR, 'pat_directory', self.patients_list[idx], 'workspace')
    ...
    def ants_atlas_registration_t1(self, path):
        ...
        output = os.path.join(self.pat_directory, pat, 'workspace')
        ...
        shutil.move(output + '/Warped.nii.gz', os.path.join(self.pat_directory, pat, 'mnispace', 't1.nii'))
        print('Done', os.path.join(self.pat_directory, pat, 'mnispace', 't1.nii'))
    ...
    def ants_atlas_registration_flair(self, path):
        ...
        print('ANTs Registration with atlas took ', round(end - start, 1), 'sec')
        shutil.move(output + '/Warped.nii.gz', os.path.join(self.pat_directory, pat, 'mnispace', 'flair.nii'))
        print('Done', os.path.join(self.pat_directory, pat, 'mnispace', 'flair.nii'))